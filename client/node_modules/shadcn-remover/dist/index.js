...
async function checkComponentsDir() {
...
async function removeComponent(componentName, dryRun = false) {
...
async function removeImportsFromFile(filePath, componentNames, dryRun) {
    let content = await fs.readFile(filePath, "utf-8");
    let newContent = content;
    for (const componentName of componentNames) {
        const importRegex = new RegExp(`import\\s+{[^}]*}\\s+from\\s+['"][^'"]*\\/components\\/ui\\/${componentName}['"];?`, "g");
        newContent = newContent.replace(importRegex, "");
    }
    if (newContent !== content) {
        const relativePath = path.relative(process.cwd(), filePath);
        if (dryRun) {
            console.log(chalk.yellow(`[Dry Run] Would remove imports from: ${relativePath}`));
        }
        else {
            await fs.writeFile(filePath, newContent, "utf-8");
            console.log(chalk.green(`Removed imports from: ${relativePath}`));
        }
    }
}
...
(async () => {
    try {
        ...
        program
            ...
            let componentsToDelete = components;
            ...
            if (removeAll) {
                ...
                const allComponents = await getAllComponents(); // Handles internal exit on error
            ...
            const proceed = await confirm(dryRun
            ...
            let successCount = 0;
            let failCount = 0;
            const results = await Promise.allSettled(componentsToDelete.map((component) => removeComponent(component, dryRun)));
            ...
            for (const file of filesToCheck) {
                await removeImportsFromFile(file, componentsToDelete, dryRun);
            }
