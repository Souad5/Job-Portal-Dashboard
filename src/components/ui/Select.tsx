...
export default function Select({ name, label, options }: SelectProps) {
  ...
  return (
    <div className="space-y-1 min-w-full sm:w-1/3">
      ...
      >
        <SelectPrimitive.Trigger className="w-full mt-1.5 flex justify-between items-center px-3 py-2 border border-gray-200 rounded-md text-md focus:outline-none focus:ring-2 focus:ring-blue-200 bg-white dark:bg-slate-700">
          ...
          <SelectPrimitive.Viewport className="p-1">
            {options.map((opt) => (
              <SelectPrimitive.Item
                key={opt}
                value={opt}
                className="relative flex items-center px-3 py-2 rounded-md text-gray-700 dark:text-white text-sm cursor-pointer hover:bg-blue-100 focus:bg-blue-200 focus:outline-none"
              >
